    ...
    async def get_credential_detail(
        ...
        *,
        ...
        return IssuerCredentialDetailData(
            ...
            dob=dob_str,
            major=getattr(cred, "major", None) or getattr(cred, "major_vi", None),
            degree_type=getattr(cred, "degree_type", None) or DEFAULT_DEGREE_TYPE,
            graduation_year=cred.graduation_year,
            classification=getattr(cred, "classification", None) or getattr(cred, "graduation_classification_vi", None),
            mode_of_study=getattr(cred, "mode_of_study_vi", None),
            university_email=cred.university_email,
            place_of_birth=getattr(cred, "place_of_birth", None),
            gender=getattr(cred, "gender", None),
            faculty=getattr(cred, "faculty", None),
            specialization=getattr(cred, "specialization", None),
            cpa=getattr(cred, "cpa", None),
            degree_number=getattr(cred, "degree_number", None),
            register_number=getattr(cred, "register_number", None),
            notes=getattr(cred, "notes", None),
            phone=cred.phone,
            status=cred.status,
