...
class InstitutionInviteService:
    ...
    async def submit_password(
        ...
        *,
        ...
        org_role = getattr(org, "type", "verifier") if org else "verifier"
    ...
    async def verify_otp(
        ...
        *,
        ...
        # 1. Validate invite link token first (propagates AppError if used/expired/invalid)
        invite_context = await validate_pending_invite(raw_token=invite_token)
        ...
        # Look up institution account info directly from raw collection
        try:
